    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-slate-900 sticky top-0 z-10">
        <div className="max-w-lg mx-auto px-4 py-4 flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-bold text-sm leading-tight">My Clients</p>
            <p className="text-slate-400 text-[10px] leading-tight">Confirmed bookings</p>
          </div>
          {!loading && (
            <span className="ml-auto text-slate-400 text-xs">{clients.length} client{clients.length !== 1 ? 's' : ''}</span>
          )}
        </div>
      </div>
